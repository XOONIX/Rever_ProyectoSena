...
namespace rever.Controllers
{
    ...
    public class EstadoPublicacionController : ControllerBase
    {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> EliminarEstadoPublicacion([FromBody] EstadoPublicacion estadopublicacion)
        {
            try
            {
                if (User.Identity == null || !User.Identity.IsAuthenticated)
                ...
                if (existe == null)
                {
                    return StatusCode(404, $"404: No se puede eliminar. El estado de publicación con ID {estadopublicacion.IdEstado} no existe.");
                }
