...
namespace rever.Controllers
{
    ...
    public class CaracteristicaController : ControllerBase
    {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> EliminarCaracteristica([FromBody] Caracteristica caracteristica)
        {
            try
            {
                if (User.Identity == null || !User.Identity.IsAuthenticated)
                ...
                if (existe == null)
                {
                    return StatusCode(404, $"404: No se puede eliminar. La característica con ID {caracteristica.IdCaracteristica} no existe.");
                }
