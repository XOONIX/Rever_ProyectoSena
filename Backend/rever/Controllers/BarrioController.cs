...
namespace rever.Controllers
{
    ...
    public class BarrioController : ControllerBase
    {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> EliminarBarrio([FromBody] Barrio barrio)
        {
            try
            {
                if (User.Identity == null || !User.Identity.IsAuthenticated)
                ...
                if (existe == null)
                {
                    return StatusCode(404, $"404: No se puede eliminar. El barrio con ID {barrio.IdBarrio} no existe.");
                }
